...
const totalSum = (input) => {
  ...
  let array=input.reduce(function(accumlator,val){
    return accumlator+=val.reduce(function(accumlator1,value){return accumlator1=accumlator1+value;
    },0);
  },0);
  return array;
};
...
let findShortest = (data) => {
  ...
  let shortestWord = data.reduce((shortest, currentWord) => {
    if (shortest.length < currentWord.name.length )
       return shortest;
    else
       return currentWord.name;
  }, "long testing string that can't be a character name");
